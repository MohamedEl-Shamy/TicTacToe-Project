...
*     Third Modification : Documentation and seesion token update.
 *
 */
//-----------------------Includes-------------------------------
#include <iostream>
...
int login(const char* db_tic_tac_toe, string username, string password, bool& loggedIn);
void updatePlayerStats(const char* db_tic_tac_toe, int userID, const string& column);
int determineWinner(char grid[3][3]);
void updateStats(const char* db_tic_tac_toe, int player1ID, int player2ID, char grid[3][3]); //calls update player stats
int insertGameData(const char* db_tic_tac_toe, int player1ID, int player2ID, char grid[3][3]);
...
void GiveUserSessionToken();
void storeSessionToken(int userID, const string& sessionToken);
void generateAndStoreSessionToken(int userID);
void DestroyCurrentSession(int userID);

//-----------------------Main-------------------------------
int main()
{
    const char* dir = "C:\\Users\\HP\\Desktop\\EECE26\\Second Year Second Term\\Embedded Systems\\StudyTest\\GameDataBase.db";
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 4: // For 2 players
            {
                ...
                while (true)
                {
                cout<<"Second payer: Enter 1 for login and 2 for registering."<<endl;
                cin >> choice2;
...
int login(const char* db_tic_tac_toe, string username, string password, bool& loggedIn)
{
    ...
    CheckPrepare = sqlite3_bind_text(stmt, 1, username.c_str(), -1, SQLITE_STATIC);
    if (CheckPrepare != SQLITE_OK)
    {
...
int registerUser(const char* db_tic_tac_toe, string username, string password)
{
    ...
    string checkExistenceSQL = "SELECT COUNT(*) FROM UsersInformation WHERE UserName = ?;";
    sqlite3_stmt* checkExistenceStmt;
    int CheckPrepare = sqlite3_prepare_v2(DB, checkExistenceSQL.c_str(), -1, &checkExistenceStmt, NULL);
    ...
    CheckPrepare = sqlite3_bind_text(checkExistenceStmt, 1, username.c_str(), -1, SQLITE_STATIC);
    if (CheckPrepare != SQLITE_OK)
    {
        cerr << "Bind error: " << sqlite3_errmsg(DB) << endl;
    ...
    string insertSQL = "INSERT INTO UsersInformation (UserName, Password) VALUES (?, ?);";
    sqlite3_stmt* insertStmt;
    CheckPrepare = sqlite3_prepare_v2(DB, insertSQL.c_str(), -1, &insertStmt, NULL);
    ...
    CheckPrepare = sqlite3_bind_text(insertStmt, 1, username.c_str(), -1, SQLITE_STATIC);
    int CheckPrepare2 = sqlite3_bind_text(insertStmt, 2, hashedPassword.c_str(), -1, SQLITE_STATIC);
    ...
    string insertStatsSQL = "INSERT INTO PlayerStats (UserID, Wins, Losses, Draws, PlayedGames) VALUES (?, 0, 0, 0, 0);";
    sqlite3_stmt* insertStatsStmt;
    CheckPrepare = sqlite3_prepare_v2(DB, insertStatsSQL.c_str(), -1, &insertStatsStmt, NULL);
...
int insertGameData(const char* db_tic_tac_toe, int player1ID, int player2ID, char grid[3][3])
{
    string sql = "INSERT INTO GamesHistory (Player1ID, Player2ID, GRID_11, GRID_12, GRID_13, GRID_21, GRID_22, GRID_23, GRID_31, GRID_32, GRID_33) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    sqlite3_stmt* stmt;
    int LocalChecker= sqlite3_prepare_v2(DB, sql.c_str(), -1, &stmt, NULL);

    if (LocalChecker != SQLITE_OK) {
        cerr << "Preparation error from insertGameData: " << sqlite3_errmsg(DB) << endl;
        return 1;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            LocalChecker = sqlite3_bind_text(stmt, i * 3 + j + 3, &grid[i][j], 1, SQLITE_STATIC);
        }
    }
...
void updateStats(const char* db_tic_tac_toe, int player1ID, int player2ID, char grid[3][3]) {
...
void updatePlayerStats(const char* db_tic_tac_toe, int userID, const string& column) {
    ...
    string sql = "UPDATE PlayerStats SET " + column + " = " + column + " + 1 WHERE UserID = ?;";
    sqlite3_stmt* stmt;
    int LocalChecker = sqlite3_prepare_v2(DB, sql.c_str(), -1, &stmt, NULL);
...
int ShowStats(const char* db_tic_tac_toe, int UserID)
{
    ...
    const char* sql = "SELECT Wins, Losses, Draws, PlayedGames FROM PlayerStats WHERE UserID = ?;";
...
string getUserName(int userID) {
    ...
    if (LocalChecker != SQLITE_OK) {
        cerr << "Failed to prepare UserName statement: " << sqlite3_errmsg(DB) << endl;
        return "";
    }
...
void storeSessionToken(int userID, const string& sessionToken)
{
    cout<<"The Session Token is "<<sessionToken<<endl;
    ...
    if (LocalChecker != SQLITE_OK) {
        cerr << "Preparation error from storeSessionToken: " << sqlite3_errmsg(DB) << endl;
        return;
    }

    LocalChecker = sqlite3_bind_text(stmt, 1, sessionToken.c_str(), -1, SQLITE_STATIC);
...
void DestroyCurrentSession(int userID) {
    ...
    string sql = "UPDATE UsersInformation SET SessionToken = NULL WHERE UserID = ?;";
    sqlite3_stmt* stmt;
    int LocalChecker = sqlite3_prepare_v2(DB, sql.c_str(), -1, &stmt, NULL);
